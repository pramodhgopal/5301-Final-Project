...
```{r}
library(ggplot2)
barplot <- ggplot(data = incidents_df, aes(x= Borough, y= Incidents)) + geom_bar(stat = "identity", fill="steelblue") + labs(title = "Total shooting related incidents by Boroughs in New York city", x = "Borough", y="Incidents")
barplot
```


Brooklyn seems to have the highest number of shooting incidents. Do we know if thats always been the case over the years? We shall plot out the number of murders over the years and group them by Boroughs. This way we can see if there are any consistencies in the Boroughs over the years from 2006.
```{r}
barplot2 <- ggplot(data = incidents_df, aes(x= Year, y= Murders, fill = Borough)) + geom_bar(stat="identity") + labs(title = "Total shooting related murders over the years in New York city", x = "Years", y="Murders")
barplot2
```
